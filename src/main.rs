...
fn tool_schemas(plan: bool) -> Vec<Value> {
    let mut tools = vec![
        ...
        json!({"type":"function","function":{"name":"web_search","description":"Search the web (DuckDuckGo) for current information the workspace cannot answer: library versions, error messages, documentation, news. Returns numbered result titles, URLs, and snippets. The query is sent to an external service; outside yolo every call requires user approval.","parameters":{"type":"object","properties":{"query":{"type":"string"},"max_results":{"type":"integer","minimum":1,"maximum":10,"description":"Results to return (default 5)."}},"required":["query"],"additionalProperties":false}}}),
    ];
    if plan {
        tools.retain(|tool| {
            matches!(
