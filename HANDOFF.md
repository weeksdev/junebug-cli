...
- An interactive REPL (`febo` with no prompt and a terminal attached) with a Claude Code-style UI:
  ...
  - slash commands: `/help`, `/status`, `/diff`, `/exit`, `/compact` (summary replaces in-memory history; session log keeps raw history), `/model` (no-arg opens an arrow-key `select_menu` over the provider's live `GET /models` list; `/model NAME` still switches by exact/substring match), and `/permissions` (arrow-key `select_menu` over read-only/ask/workspace-write/yolo, switchable mid-session; rebuilds the `PolicyEngine` each turn from a mutable `permission` local in `repl`);
  - a dimmed status line under the prompt (`editor::read_line(footer)`) always shows the current model and effective access level;
  - `editor::select_menu(title, choices, initial)` is the shared raw-mode arrow-key picker (up/down or j/k, enter/tab accept, esc/ctrl-c cancel), used by `/model` and `/permissions`;
