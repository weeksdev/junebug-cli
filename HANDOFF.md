...
- API keys can be supplied through environment variables, a local ignored `.env` file, or the user-level `~/.febo/credentials.env` written by `febo set --provider NAME API_KEY` (0600 on unix; the command saves the key and then starts the REPL on that provider). Lookup order: process env, workspace `.env`, user credentials file. Supported names are `openrouter`, `openai`, `deepseek`, and `anthropic` (`claude` is accepted as an alias).
...
- An interactive REPL (`febo` with no prompt and a terminal attached) with a Claude Code-style UI:
  ...
  - slash commands: `/help`, `/status`, `/diff`, `/exit`, `/compact` (summary replaces in-memory history; session log keeps raw history), `/model` (no-arg opens one arrow-key menu grouped by every provider with a credential and lists each provider's live `GET /models` results below it; selecting a model switches provider in one step; `/model provider:model` remains available), and `/permissions` (arrow-key `select_menu` over read-only/ask/workspace-write/yolo, switchable mid-session; rebuilds the `PolicyEngine` each turn from a mutable `permission` local in `repl`);
  - a dimmed status line under the prompt (`editor::read_line(footer)`) always shows the current model and effective access level;
...
- The `--resume` picker: `--resume`/`--resume-compact` with no path (and a TTY) lists this workspace's sessions newest-first with age + first-prompt preview (`session::list_sessions`) and reads a numbered choice. An explicit path still resumes directly; a path is only consumed when the next arg is an existing file.
- Provider defaulting: `--provider` is optional. When omitted, `resolve_provider_kind` uses the provider recorded in the newest session (`session::last_provider`; each run records a `provider` event), else the sole provider with a credential, else prints how to set one and exits. `provider::available_providers()` / `ProviderKind::has_credential()` check env + `.env` + `~/.febo/credentials.env`.
- `febo set --provider NAME KEY` saves a credential to `~/.febo/credentials.env` (0600) then launches the REPL on that provider.
- Safety controls:
  ...
  - `yolo` (a.k.a. `approve-all`) pre-approves every write AND command with no prompt; plan mode still forces read-only over it. Set via `--permission yolo` or the `/permissions` menu, which warns in red when chosen;
